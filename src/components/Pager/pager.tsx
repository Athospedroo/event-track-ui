...
function Pager({ itemsCount, handleChangePage, validatePage, interval = 10, filter, trigger = false, synchronized = false, showPages = true }: Props) {
  ...
  function createPages() {
    ...
    if (pagesCount < MAX_INDEX) {
      ...
      && (currentPage.current + 1 <= pagesCount - (MAX_INDEX - 1))) {
      pages.push('<<')
  ...
  function ShowPage(page: any) {
    switch (page) {
      case '<<':
        return (
          <button className="btn btn-outline-secondary rounded-pill border-0" onClick={() => SelectedPage(page)}>
      ...
      case currentPage.current+1:
        return (
          <button style={{backgroundColor: '#c0ccf3'}} className="btn btn-outline-secondary rounded-pill border-0 text-dark" onClick={() => SelectedPage(page)}>
            {page}
          </button>
        )
      default:
